...
export class GrindSheet extends Application {
  ...
  async _onSortPartyMember(event, actorData) {
    ...
    const source = {
      data: { type: "Character", _id: actorData._id, sort: this._grindData.grind.actors.indexOf(actorData._id) },
    };
  ...
  _getSortSiblings(source) {
    return this._grindData.grind.actors
      .filter((aId) => {
  ...
  computeLightLevels(ambientLight, tbActors) {
    ...
    for (let i = 0; i < tbActors.length; i++) {
      ...
      for (
        let powerIdx = 0;
        powerIdx < emittedLight.characters && sequenceIdx < LIGHT_SEQUENCE.length;
        powerIdx++, sequenceIdx++
      ) {
      ...
      for (
        let powerIdx = 0;
        powerIdx < emittedLight.dim && sequenceIdx < LIGHT_SEQUENCE.length;
        powerIdx++, sequenceIdx++
      ) {
        let litCharacterIndex = i + LIGHT_SEQUENCE[sequenceIdx];
        while ((litCharacterIndex < 0 || litCharacterIndex >= tbActors.length) && sequenceIdx < LIGHT_SEQUENCE.length) {
        ...
        if (litCharacterIndex >= 0 && litCharacterIndex < tbActors.length) {
          lightLevels[tbActors[litCharacterIndex].data._id] = Math.max(
            1,
            lightLevels[tbActors[litCharacterIndex].data._id]
          );
        }
      }
    }
...
Handlebars.registerHelper("renderGrindActor", function (actorId, lightLevels) {
  ...
  let html = "";
  html += `<li class="grind actor" style="display:flex;align-items: center;background-color: ${bgColor}" data-actor-id="${actorId}">`;

  //Actor name & icon
  html += `<div style="display: flex;align-items: center;flex:1;;color:${nameColor}">
  ...
  //Light summary
  html += `<div style="display:flex;align-items: center;flex:1;padding-left:8px;">`;
  tbData.computed.emittedLight.held.forEach((i) => {
    ...
    for (let consumeIdx = 0; consumeIdx < i.data.lightsource.remaining; consumeIdx++) {
      html += `<a class="item-control item-consume" title="Consume" style="margin-right: 5px;"><i style="${i.data.consumable.iconStyle};" class="fas ${i.data.consumable.icon}"></i></a>`;
    }
  });
  tbData.computed.emittedLight.tossed.forEach((i) => {
    html += `<img src='systems/torchbearer/assets/icons/items/${i.name.toLowerCase()}.png' title='${
      i.name
  ...
  //Conditions summary
  html += `<div style="display:flex;align-items: center;flex:2;padding-left:8px;">`;
