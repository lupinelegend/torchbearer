...
export class HelpingDiceDialog extends Dialog {
    static async create(tbActor, opts, onComplete) {
        ...
        let partyMembers = (await CurrentCharacterActorIds()).filter(id => {
        ...
        let template = await renderTemplate(dialogContent,
            Object.assign({
                allSkills, partyMembers, helpSkillAbility1, helpSkillAbility2, helpSkillAbility3, helpSkillAbility4,
            }, opts)
        );
    ...
    fillOutOptions(html) {
        ...
        html.find('.party-member').each((i, el) => {
            ...
            if(tbActor.tbData().afraid) {
                ...
                $row.find('.wise').html("");
                $row.find('.name').html($row.find('.name').html() + "<img src='systems/torchbearer/assets/icons/conditions/afraid.png' title='afraid' style='height:16px;margin-left:4px;border:0;' />");
            } else {
